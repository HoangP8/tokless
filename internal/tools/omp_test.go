...
func TestOmpToolWiring(t *testing.T) {
	...
	if !strings.Contains(string(ext), `import { spawn } from "node:child_process"`) || !strings.Contains(string(ext), `spawn(TOKLESS, ["rtk-hook", "omp"], { stdio: ["pipe", "pipe", "ignore"] })`) || !strings.Contains(string(ext), `child.stdin.end(JSON.stringify(payload))`) || strings.Contains(string(ext), "setTimeout") || !strings.Contains(string(ext), `child.once("error", () => finish())`) || !strings.Contains(string(ext), `child.stdin.once("error", () => finish())`) || !strings.Contains(string(ext), `child.once("close", (code) => finish(code === 0 ? output : ""))`) || !strings.Contains(string(ext), `if (settled) return`) || strings.Contains(string(ext), `pi.exec(`) || strings.Contains(string(ext), `"sh"`) || strings.Contains(string(ext), `{ input: JSON.stringify(payload) }`) {
