...
└─ NO  → Evaluate before falling back:
         1. Tiny project (e.g., <= 5 source files and flat directory)? → Skip init. Fallback to normal (read/grep).
         2. Complex project (e.g., > 5 source files or nested subdirectories)? → You MUST actively execute the terminal command `codegraph init` to bootstrap. (Note: This does NOT violate ponytail).
